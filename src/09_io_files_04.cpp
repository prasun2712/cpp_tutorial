...
tellg() and tellp()
-------------------
These two member functions with no parameters return a value of the member type streampos, which is a type representing the current get position 
...
offset is of type streamoff. And direction is of type seekdir, which is an enumerated type that determines the point from where offset is counted from,
...
{
    streampos begin, end;
    ifstream myfile("../data/NotoSansShavian-Regular.bin", ios::binary);
    begin = myfile.tellg();
    myfile.seekg(0, ios::end);
    end = myfile.tellg();
    myfile.close();
    cout << "size is: " << float(end - begin)/1000 << " kB.\n";
    return 0;
}