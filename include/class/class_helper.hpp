...
In the below example, all three classes (Polygon, Rectangle and Triangle) have the same members: width, height, and functions set_values and area.
...
i.e., if virtual is removed from the declaration of area in the example above, all three calls to area would return zero, because in all cases,
...
as one in the base class to be appropriately called from a pointer, and more precisely when the type of the pointer is a pointer to the base class
that is pointing to an object of the derived class, as in the below example.
...
class VirtualRectangle : public VirtualPolygon
{
public:
  double area();
};

class VirtualTriangle : public VirtualPolygon
{
public:
  double area();
};
